...
function cardGenerator() {
  ...
  const randomNumber = numbers[Math.floor(Math.random() * numbers.length)]; // Guardo el elemento de numbers aleatorio
  const randomCardType = cardType[Math.floor(Math.random() * cardType.length)]; // Guardo el elemento aleatorio
  let suit = "";
  ...
  centralNumber.innerHTML = randomNumber;
  ...
  cardElement.classList.add(randomCardType);
}
